...
class Floor:
    ...
    def color_button(self):
        ...
        if not self.time_to_wait or not self.time_to_wait.time_left():# there no eevetor on the way
            return (170, 170, 170)# Gray color for inactive button
        return (0, 255, 0)# Green color for active button
    ...
    def draw_timer(self, window):
        ...
        if self.time_to_wait:  # the timer instantiate
            if self.time_to_wait.time_left():  # the clock is runing
                ...
                font = pygame.font.SysFont("Arial", int(FONT_SIZE/2))
                # the current time from the timer in red on green background
                txt_surf = font.render(time_now, True, (255, 0, 0), (0, 255, 0))
    ...
    def draw_button(self,window):
        ...
        txt_surf = font.render(str(self.num_of_floor), True, (0, 0, 0))
